...
class ArsObstacleAvoidanceReact:
  ...
  def ctrLoopObstacleAvoidanceReact(self, time_stamp_ros):
    ...
    for obst_detected_i in self.obstacles_detected_msg.markers:
      ...
      self.distance_of_influence = 2.0 # (d_inf)
      self.gain_avoidance = 3.0  # (K)

      d_i = math.sqrt(abs(posi_2d_obst_i_wrt_robot[0])**2 + abs(posi_2d_obst_i_wrt_robot[1])**2)

      d_ef_i = d_i - (rad_obst_i + self.robot_size_radius)

      n_i = - (1 / (math.sqrt(posi_2d_obst_i_wrt_robot[0]**2 + posi_2d_obst_i_wrt_robot[1]**2))) * np.array([posi_2d_obst_i_wrt_robot[0], posi_2d_obst_i_wrt_robot[1]])
      
      if d_ef_i >= self.distance_of_influence:     
        u_rep_i = 0
        self.robot_velo_lin_cmd_avoid[0] += n_i[0]*u_rep_i
        self.robot_velo_lin_cmd_avoid[1] += n_i[1]*u_rep_i

      elif 0 < d_ef_i < self.distance_of_influence:   

        u_rep_i = - self.gain_avoidance * math.log10(d_ef_i/self.distance_of_influence)         
        self.robot_velo_lin_cmd_avoid[0] += n_i[0]*u_rep_i
        self.robot_velo_lin_cmd_avoid[1] += n_i[1]*u_rep_i

      else: #infinity
        u_rep_i = 65 #float('inf')
        self.robot_velo_lin_cmd_avoid[0] += n_i[0]*65 #float('inf')
        self.robot_velo_lin_cmd_avoid[1] += n_i[1]*65 #float('inf')
